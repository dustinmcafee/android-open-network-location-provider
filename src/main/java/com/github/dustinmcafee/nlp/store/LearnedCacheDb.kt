 ...
 */
internal class LearnedCacheDb(context: Context) :
     ...
     */
    fun observe(bssid: String, latDeg: Double, lngDeg: Double, nowEpochSec: Long) {
        ...
        try {
            ...
            if (existing == null) {
                ...
                val distMeters = haversineMeters(
                    newLatE8.toDouble() / 1e8, newLngE8.toDouble() / 1e8,
                    latDeg, lngDeg,
                ).toFloat()
                val newDisp = existing.dispersionMeters +
    ...
    companion object {
        ...
        private val COLUMNS = arrayOf(
            COL_BSSID, COL_LAT_E8, COL_LNG_E8,
            COL_DISPERSION, COL_SAMPLE_COUNT, COL_LAST_UPDATED,
        )
